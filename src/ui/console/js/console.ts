...
interface DOMElements {
  ...
  activeAgents: HTMLElement | null;
  uptime: HTMLElement | null;
  memoryUsage: HTMLElement | null;
  messageCount: HTMLElement | null;
  timestamp: HTMLElement | null;
  clearConsole: HTMLButtonElement | null;
...
interface StreamingOutputParams {
  content: string;
  type?: string;
  streaming?: boolean;
}
...
interface ConnectionEstablishedParams {
  server?: string;
  version?: string;
}
...
class ClaudeCodeConsole {
  ...
  private terminal: TerminalEmulator | null;
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      this.showError('Failed to initialize console: ' + errorMessage);
    }
  }
  ...
  private getDOMElements(): void {
    ...
    const required: (keyof DOMElements)[] = ['consoleOutput', 'consoleInput', 'loadingOverlay'];
  ...
  private setupComponentInteractions(): void {
    if (!this.terminal || !this.commandHandler) return;
    ...
    this.wsClient.on('disconnected', (info: DisconnectionInfo) => {
    ...
    this.wsClient.on('reconnecting', (info: { attempt: number }) => {
      this.updateConnectionStatus(false, true);
      this.terminal!.writeInfo(`Reconnecting... (${info.attempt}/${(this.wsClient as any).maxReconnectAttempts})`);
    });
    ...
    this.wsClient.on('message_received', (message: any) => {
      this.messageCount++;
      this.handleIncomingMessage(message);
    });
    
    this.wsClient.on('notification', (notification: NotificationParams) => {
      this.handleNotification(notification);
    });
    
    // Settings -> Application
    this.settings.on('connect_requested', async (config: { url: string; token: string }) => {
    ...
    this.settings.on('setting_changed', ({ key, value }: { key: string; value: any }) => {
   ...
   */
  private handleNotification(notification: NotificationParams): void {
   ...
   * Handle streaming output
   */
  private handleStreamingOutput(params: StreamingOutputParams): void {
    if (params && params.content && this.terminal) {
   ...
   * Handle Claude Flow notifications
   */
  private handleClaudeFlowNotification(message: { method: string; params: any }): void {
   ...
   */
  private handleAgentStatus(params: AgentStatusParams): void {
    if (params.active !== undefined) {
      this.activeAgents = params.active;
    }
    
    if (params.message && this.terminal) {
   ...
   */
  private handleSwarmUpdate(params: SwarmUpdateParams): void {
    if (params.message && this.terminal) {
   ...
   */
  private handleMemoryUpdate(params: MemoryUpdateParams): void {
    if (params.message && this.terminal) {
   ...
   * Handle log messages
   */
  private handleLogMessage(params: LogMessageParams): void {
    if (params.level && params.message && this.terminal) {
   ...
   */
  private handleConnectionEstablished(params: ConnectionEstablishedParams): void {
  ...
  private updateStatus(): void {
    ...
    if (this.elements.memoryUsage && (performance as any).memory) {
  ...
  private setupEventListeners(): void {
    ...
    window.addEventListener('unhandledrejection', (event: PromiseRejectionEvent) => {
      ...
      if (this.terminal) {
        ...
        this.terminal.writeError(`Unhandled error: ${reason}`);
      }
    });
    ...
    window.addEventListener('error', (event: ErrorEvent) => {
      ...
      if (this.terminal) {
        ...
        this.terminal.writeError(`Application error: ${message}`);
      }
    });
  }
