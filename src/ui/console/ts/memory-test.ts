...
export class MemoryTest {
  ...
  constructor(memoryManager: MemoryManager, terminal: Terminal) {
  ...
  async runTests(): Promise<void> {
    ...
    try {
      ...
      this.terminal.writeError(`❌ Memory tests failed: ${errorMessage}`);
    }
  }
   ...
   */
  async testMemoryPanel(): Promise<void> {
    this.terminal.writeInfo('Testing memory panel...');
    
    // Test panel toggle
    this.memoryManager.togglePanel();
    const memoryPanel = document.getElementById('memoryPanel');
    const panelVisible = memoryPanel ? !memoryPanel.classList.contains('hidden') : false;
   ...
   * Test memory tools
   */
  async testMemoryTools(): Promise<void> {
    ...
    for (const tool of tools) {
      const toolInfo: MemoryToolInfo = this.memoryManager.memoryTools[tool];
   ...
   */
  async testNamespaceOperations(): Promise<void> {
  ...
  async testMockData(): Promise<void> {
    ...
    const visibleRows = rows ? Array.from(rows).filter(row => row.style.display !== 'none') : [];
...
export async function runMemoryTests(): Promise<TestReport | null> {
  const windowWithGlobals = window as any;
  
  if (windowWithGlobals.memoryManager && windowWithGlobals.claudeConsole) {
    const tester = new MemoryTest(
      windowWithGlobals.memoryManager, 
      windowWithGlobals.claudeConsole.terminal
    );
