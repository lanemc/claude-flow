...
// Type definitions for EventBus
export interface EventHandler {
  (data?: any, eventInfo?: EventInfo): void | Promise<void>;
}
...
export class EventBus {
  private events: Map<string, EventHandlerInfo[]>;
  private onceEvents: Map<string, EventHandlerInfo[]>;
  private wildcardHandlers: Map<string, EventHandlerInfo[]>;
   ...
   */
  on(event: string, handler: EventHandler, context: any = null): () => boolean {
   ...
   */
  once(event: string, handler: EventHandler, context: any = null): () => boolean {
    if (typeof handler !== 'function') {
      throw new Error('Event handler must be a function');
    }
   ...
   */
  onWildcard(pattern: string, handler: EventHandler, context: any = null): () => boolean {
    if (typeof handler !== 'function') {
      throw new Error('Event handler must be a function');
    }
   ...
   */
  offOnce(event: string, handler: EventHandler): boolean {
   ...
   */
  offWildcard(pattern: string, handler: EventHandler): boolean {
   ...
   */
  async executeHandlerAsync(handlerInfo: EventHandlerInfo, data: any, eventInfo: EventInfo): Promise<any> {
    try {
      let result: any;
      if (handlerInfo.context) {
   ...
   * Get event history
   */
  getEventHistory(limit = 100): EventInfo[] {
   ...
   */
  getEventsByPattern(pattern: string, limit = 100): EventInfo[] {
    const regex = this.createWildcardRegex(pattern);
   ...
   */
  createWildcardRegex(pattern: string): RegExp {
