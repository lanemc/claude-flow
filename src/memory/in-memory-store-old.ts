...
import type { MemoryStoreOptions, StoreOptions, StoreResult, MemoryEntry } from './types.js';

class InMemoryStore {
  private options: MemoryStoreOptions;
  private data: Map<string, Map<string, MemoryEntry>>; // namespace -> Map(key -> entry)
  ...
  private _getNamespaceMap(namespace: string): Map<string, MemoryEntry> {
  ...
  async retrieve(key: string, options: StoreOptions = {}): Promise<any> {
    ...
    return this._tryParseJson(entry.value);
  }
