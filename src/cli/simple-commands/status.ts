// status.ts - System status and monitoring commands
import { printSuccess, printError, printWarning } from '../utils.js';
import { Deno } from '../node-compat.js';

// Type definitions for status commands
...
export async function statusCommand(subArgs: string[], flags: StatusFlags): Promise<void> {
  const verbose: boolean = subArgs.includes('--verbose') || subArgs.includes('-v') || flags.verbose || false;
  const json: boolean = subArgs.includes('--json') || flags.json || false;
  
  const status: SystemStatus = await getSystemStatus(verbose);
...
async function getSystemStatus(verbose = false): Promise<SystemStatus> {
...
async function getResourceUsage(): Promise<ResourceUsage | null> {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        return {
          memory: { total: 'N/A', free: 'N/A', available: 'N/A', usage: 'N/A (os module unavailable)' },
          cpu: { cores: 0, load: 'Unknown', model: 'Unknown' },
          platform: { type: 'Unknown', release: 'Unknown', arch: 'Unknown', uptime: 'Unknown', error: 'os module not available' }
        };
      }
    }
    ...
    let loadAvg: string = 'N/A';
    
    try {
      const loadAvgData: number[] = os.loadavg();
    ...
    return {
      memory: { total: 'Unknown', free: 'Unknown', available: 'Unknown', usage: 'Unknown' },
      cpu: { cores: 0, load: 'Unknown', model: 'Unknown' },
      platform: { type: 'Unknown', release: 'Unknown', arch: 'Unknown', uptime: 'Unknown', error: error.message }
    };
  }
}

function displayStatus(status: SystemStatus, verbose: boolean): void {
  printSuccess('Claude-Flow System Status:');
  
  // Overall status
  const overallStatus: string = status.orchestrator.running ? '🟢 Running' : '🟡 Not Running';
...
// @ts-expect-error - import.meta.main is Deno-specific
