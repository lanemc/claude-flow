...
// Type definitions for coordination commands
...
export async function coordinationAction(subArgs: string[], flags: CoordinationFlags): Promise<void> {
...
async function swarmInitCommand(subArgs: string[], flags: CoordinationFlags): Promise<void> {
    const options = flags;
    const swarmId: string = options['swarm-id'] || options.swarmId || generateId('swarm');
    const topology: string = options.topology || 'hierarchical';
    const maxAgents: number = parseInt(String(options['max-agents'] || options.maxAgents || '5'));
    ...
    const isAvailable: boolean = await checkRuvSwarmAvailable();
    if (!isAvailable) {
    ...
    try {
        ...
        const swarmInitOptions: SwarmInitOptions = {
            swarmId,
            topology,
            maxAgents,
            strategy,
        ...
        const swarmResult: SwarmResult = await callRuvSwarmMCP('swarm_init', swarmInitOptions) as SwarmResult;
...
async function agentSpawnCommand(subArgs: string[], flags: CoordinationFlags): Promise<void> {
    const options = flags;
    const agentType: string = options.type || subArgs[1] || 'general';
    const agentName: string = options.name || `${agentType}-${generateId('agent')}`;
    const swarmId: string | undefined = options['swarm-id'] || options.swarmId;
    const capabilities: string | null = options.capabilities || null;
    ...
    // Validate agent type
    const validTypes: string[] = ['coordinator', 'coder', 'researcher', 'analyst', 'tester', 'general'];
...
async function taskOrchestrateCommand(subArgs: string[], flags: CoordinationFlags): Promise<void> {
    const options = flags;
    const task: string = options.task || subArgs.slice(1).join(' ');
    const swarmId: string | undefined = options['swarm-id'] || options.swarmId;
    const strategy: string = options.strategy || 'adaptive';
    const shareResults: boolean = options['share-results'] || false;
...
function showCoordinationHelp(): void {
    console.log(`
