/**
 * Enhanced Process UI - Enhanced process management UI with multiple views in TypeScript
 * Converted from JavaScript with comprehensive type definitions
 */
...
// Type definitions for enhanced UI functionality
...
type ViewType = 'processes' | 'status' | 'orchestration' | 'memory' | 'logs' | 'help';
...
export class EnhancedProcessUI {
  ...
  private swarmIntegration: SwarmWebUIIntegration;
  ...
  private renderProcessView(): void {
    ...
    for (const [id, processInfo] of Array.from(this.processes.entries())) {
      ...
      if (processInfo.status === 'running') {
        const stats = colors.dim(`PID: ${processInfo.pid} | Uptime: ${this.formatUptime(processInfo.uptime)} | CPU: ${processInfo.cpu.toFixed(1)}% | Mem: ${processInfo.memory.toFixed(0)}MB`);
        console.log(`     ${stats}`);
      }
      console.log();
    ...
    console.log(colors.gray('─'.repeat(80)));
    console.log(colors.white(`Total: ${this.processes.size} | Running: ${colors.green(running.toString())} | Stopped: ${colors.gray((this.processes.size - running).toString())}`));
  }
  ...
  private renderStatusView(): void {
    ...
    this.logs.slice(-3).forEach((log: LogEntry) => {
  ...
  private renderOrchestrationView(): void {
    ...
    console.log();
    this.agents.forEach((agent: Agent, index: number) => {
    ...
    console.log();
    this.tasks.slice(0, 5).forEach((task: Task) => {
      ...
      if (task.assignedTo) {
        const agent = this.agents.find((a: Agent) => a.id === task.assignedTo);
  ...
  private renderMemoryView(): void {
    ...
    console.log();
    this.memoryStats.namespaces.forEach((ns: MemoryNamespace, index: number) => {
  ...
  private renderLogsView(): void {
    ...
    displayLogs.forEach((log: LogEntry) => {
      const time = log.time.toLocaleTimeString();
  ...
  private getUsageBar(value: number, max: number): string {
  ...
  private async handleViewSpecificInput(input: string): Promise<void> {
    switch (this.currentView) {
      case VIEWS.PROCESSES:
  ...
  private async handleProcessInput(input: string): Promise<void> {
    switch (input) {
      case 'a':
        await this.startAll();
        break;
  ...
  private async handleOrchestrationInput(input: string): Promise<void> {
    switch (input) {
      case 'n':
        // Spawn new agent
      ...
      case 'd':
        ...
        if (this.tasks.length > 0) {
          const pendingTasks = this.tasks.filter((t: Task) => t.status === 'in_progress');
  ...
  private async handleMemoryInput(input: string): Promise<void> {
    switch (input) {
      case 's':
  ...
  private async handleLogsInput(input: string): Promise<void> {
    switch (input) {
      case 'l':
        this.logs = [];
  ...
  private updateSystemStats(): void {
    ...
    for (const [id, processInfo] of Array.from(this.processes.entries())) {
      if (processInfo.status === 'running') {
        processInfo.uptime++;
        processInfo.cpu = Math.min(100, Math.max(0, processInfo.cpu + (Math.random() - 0.5) * 2));
        processInfo.memory = Math.min(200, Math.max(10, processInfo.memory + (Math.random() - 0.5) * 5));
      }
    }
  }
