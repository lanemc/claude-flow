// start-wrapper.ts - Wrapper to maintain backward compatibility with the new modular start command
import { printSuccess, printError, printWarning, printInfo } from '../utils';
import { Deno, cwd, exit, existsSync } from '../node-compat';
import { compat } from '../runtime-detector';
  ...
  try {
    ...
    if (web) {
      try {
        ...
        printError('Failed to launch web UI: ' + (err as Error).message);
    ...
    if (ui && !web) {
      try {
        ...
        try {
          ...
          printError('Failed to launch UI: ' + (err as Error).message);
          console.error((err as Error).stack);
          return;
        }
      }
    ...
    if (daemon) {
      ...
      await compat.safeCall(async () => {
        if (compat.runtime === 'deno') {
          await (Deno as any).writeTextFile('.claude-flow.pid', pid.toString());
        } else {
    ...
    printError(`Failed to start orchestration system: ${(err as Error).message}`);
...
async function cleanup(): Promise<void> {
  ...
  try {
    await compat.safeCall(async () => {
      if (compat.runtime === 'deno') {
        await (Deno as any).remove('.claude-flow.pid');
      } else {
        const fs = await import('fs/promises');
