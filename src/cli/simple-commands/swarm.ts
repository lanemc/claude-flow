import { args, mkdirAsync, writeTextFile, exit, cwd } from '../node-compat';
import { spawn, execSync, ChildProcess } from 'child_process';
import { existsSync, chmodSync, statSync } from 'fs';
import { open } from 'fs/promises';
import process from 'process';
import path from 'path';
export async function swarmCommand(args: string[], flags: SwarmFlags): Promise<void> {
...
The swarm should be self-documenting - use memory_store to save all important information, decisions, and results throughout the execution.`;
      ...
      console.error('❌ Failed to spawn Claude Code:', (error as Error).message);
...
`;
      ...
      try {
        ...
        const output = await (bgProcess as any).output();
...
`;
    ...
    if (fs.existsSync(bgScriptPath)) {
      ...
      try {
        ...
        console.error('Failed to launch background swarm:', (error as Error).message);
      }
    } else {
  ...
  try {
    ...
    if (error.code === 'ERR_MODULE_NOT_FOUND' || error.code === 'ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING' || error.code === 'ERR_UNKNOWN_FILE_EXTENSION') {
      ...
      try {
        ...
        console.log(`⚠️  Swarm executor error: ${(execError as Error).message}`);
        // If swarm executor fails, try to create files directly
        try {
          await createSwarmFiles(objective, flags);
          return;
        } catch (createError) {
          console.log(`⚠️  Direct file creation error: ${(createError as Error).message}`);
      ...
      try {
        ...
        let swarmDemoPath: string | null = null;
        ...
        if (swarmDemoPath && (global as any).Deno) {
          ...
          swarmProcess.on('error', (err: Error) => {
            console.error('❌ Failed to launch swarm:', err.message);
          });
          
          swarmProcess.on('exit', (code: number | null) => {
            if (code !== 0) {
...
async function createSwarmFiles(objective: string, flags: SwarmFlags): Promise<void> {
  const fs = await import('fs');
  const path = await import('path');
  ...
  for (let i = 0; i < process.argv.slice(2).length; i++) {
    ...
    if (arg.startsWith('--')) {
      ...
      if (nextArg && !nextArg.startsWith('--')) {
        (flags as any)[flagName] = nextArg;
        i++; // Skip the next argument
      } else {
        (flags as any)[flagName] = true;
      }
    } else {
      args.push(arg);
    }
