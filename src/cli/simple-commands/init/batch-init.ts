...
async function initializeProject(projectPath: string, options: ProjectOptions = {}): Promise<ProjectInitResult> {
  const {
    template = null,
    environment = 'dev',
    sparc = false,
      ...
      if (templateConfig.extraFiles) {
        for (const [filePath, content] of Object.entries(templateConfig.extraFiles)) {
          let fileContent = typeof content === 'object' ? 
            JSON.stringify(content, null, 2) : content as string;
          
          // Replace template variables
          const templateVars: TemplateVariables = {
            PROJECT_NAME: projectPath.split('/').pop() || projectPath,
            PROJECT_DESCRIPTION: templateConfig.description,
            ENVIRONMENT: environment
          };
...
export async function batchInitCommand(projects: string[], options: BatchInitOptions = {} as BatchInitOptions): Promise<ProjectInitResult[]> {
  const {
    parallel = true,
    maxConcurrency = INIT_CONSTANTS.DEFAULT_MAX_CONCURRENCY,
    template = null,
    environments = INIT_CONSTANTS.DEFAULT_ENVIRONMENTS,
    sparc = false,
    minimal = false,
    force = false,
  ...
  const results: ProjectInitResult[] = [];
...
interface BatchConfig {
  ...
  projectConfigs?: Record<string, ProjectOptions>;
}
...
export async function batchInitFromConfig(configFile: string, options: BatchInitOptions = {} as BatchInitOptions): Promise<ProjectInitResult[] | undefined> {
  ...
  const mergedOptions: BatchInitOptions = { ...baseOptions, ...options };

  // If projectConfigs are specified, use them for individual project customization
  if (Object.keys(projectConfigs).length > 0) {
    const results: ProjectInitResult[] = [];
    const resourceManager = new ResourceManager(mergedOptions.maxConcurrency || INIT_CONSTANTS.DEFAULT_MAX_CONCURRENCY);

    for (const [projectName, projectConfig] of Object.entries(projectConfigs)) {
      const projectOptions: ProjectOptions = { ...mergedOptions, ...projectConfig };
      const result = await resourceManager.withResource(async () => {
