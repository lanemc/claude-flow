// executable-wrapper.ts - Create local executable wrapper

import { writeFile, chmod } from 'fs/promises';
import { platform } from 'os';

/**
 * Create a local executable wrapper for claude-flow
 * @param workingDir - The directory where the wrapper should be created
 * @param dryRun - Whether to perform a dry run without actually creating files
 */
export async function createLocalExecutable(workingDir: string, dryRun: boolean = false): Promise<void> {
  try {
    if (platform() === 'win32') {
