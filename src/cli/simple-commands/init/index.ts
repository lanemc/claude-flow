 ...
 */
async function setupMcpServers(dryRun: boolean = false): Promise<void> {
...
export async function initCommand(subArgs: string[], flags: InitFlags): Promise<void> {
  ...
  try {
    ...
    const files = [FILE_PATHS.CLAUDE_MD, 'memory-bank.md', 'coordination.md'];
    ...
    if (!initDryRun) {
      await Deno.writeTextFile(FILE_PATHS.CLAUDE_MD, claudeMd);
      console.log(`  ✓ Created ${FILE_PATHS.CLAUDE_MD}${initOptimized ? ' (Batchtools-optimized SPARC)' : initSparc ? ' (SPARC-enhanced)' : ''}`);
    } else {
      console.log(`  [DRY RUN] Would create ${FILE_PATHS.CLAUDE_MD}${initOptimized ? ' (Batchtools-optimized SPARC)' : initSparc ? ' (SPARC-enhanced)' : ''}`);
    }
    ...
    if (!initDryRun) {
      await Deno.writeTextFile(FILE_PATHS.MEMORY_BANK_MD, memoryBankMd);
    ...
    if (!initDryRun) {
      await Deno.writeTextFile(FILE_PATHS.COORDINATION_MD, coordinationMd);
    ...
    const directories = [...DIRECTORY_STRUCTURE];
    ...
    if (initDryRun) {
      ...
      // Update .gitignore
      const gitignoreResult: GitignoreUpdateResult = await updateGitignore(workingDir, initForce, initDryRun);
...
async function handleBatchInit(subArgs: string[], flags: InitFlags): Promise<void> {
  try {
 ...
 */
async function enhancedInitCommand(subArgs: string[], flags: InitFlags): Promise<void> {
  ...
  try {
    ...
    await performInitializationWithCheckpoints(rollbackSystem, options, workingDir, false);
 ...
 */
async function handleValidationCommand(subArgs: string[], flags: InitFlags): Promise<void> {
  const workingDir = Deno.env.get('PWD') || Deno.cwd();
 ...
 */
async function handleRollbackCommand(subArgs: string[], flags: InitFlags): Promise<void> {
  const workingDir = Deno.env.get('PWD') || Deno.cwd();
  const rollbackSystem = new RollbackSystem(workingDir);
  ...
  try {
    ...
    if (subArgs.includes('--full')) {
      ...
      const rollbackPoints: RollbackPoints = await rollbackSystem.listRollbackPoints();
 ...
 */
async function handleListBackups(subArgs: string[], flags: InitFlags): Promise<void> {
  const workingDir = Deno.env.get('PWD') || Deno.cwd();
  const rollbackSystem = new RollbackSystem(workingDir);
 ...
 */
async function performInitializationWithCheckpoints(rollbackSystem: any, options: any, workingDir: string, dryRun: boolean = false): Promise<void> {
...
async function createInitialFiles(options: any, workingDir: string, dryRun: boolean = false): Promise<void> {
  if (!dryRun) {
    const claudeMd = options.sparc ? createSparcClaudeMd() : 
                     options.minimal ? createMinimalClaudeMd() : createFullClaudeMd();
    await Deno.writeTextFile(`${workingDir}/${FILE_PATHS.CLAUDE_MD}`, claudeMd);

    const memoryBankMd = options.minimal ? createMinimalMemoryBankMd() : createFullMemoryBankMd();
    await Deno.writeTextFile(`${workingDir}/${FILE_PATHS.MEMORY_BANK_MD}`, memoryBankMd);

    const coordinationMd = options.minimal ? createMinimalCoordinationMd() : createFullCoordinationMd();
    await Deno.writeTextFile(`${workingDir}/${FILE_PATHS.COORDINATION_MD}`, coordinationMd);
  }
}

async function createDirectoryStructure(workingDir: string, dryRun: boolean = false): Promise<void> {
  const directories = [
...
async function setupMemorySystem(workingDir: string, dryRun: boolean = false): Promise<void> {
  if (!dryRun) {
...
async function setupCoordinationSystem(workingDir: string, dryRun: boolean = false): Promise<void> {
 ...
 */
async function enhancedClaudeFlowInit(flags: InitFlags, subArgs: string[] = []): Promise<void> {
  ...
  try {
    ...
    const existingFiles: string[] = [];
    const filesToCheck = [FILE_PATHS.CLAUDE_MD, FILE_PATHS.CLAUDE_SETTINGS, FILE_PATHS.MCP_CONFIG, FILE_PATHS.CLAUDE_FLOW_CONFIG];
    ...
    // Create CLAUDE.md
    if (!dryRun) {
      await Deno.writeTextFile(`${workingDir}/${FILE_PATHS.CLAUDE_MD}`, createEnhancedClaudeMd());
      printSuccess(`✓ Created ${FILE_PATHS.CLAUDE_MD} (Claude Flow v2.0.0)`);
    } else {
      console.log(`[DRY RUN] Would create ${FILE_PATHS.CLAUDE_MD} (Claude Flow v2.0.0)`);
    }
    ...
    const settingsLocal: SettingsLocal = {
      "permissions": {
        "allow": [
          "mcp__ruv-swarm",
          "mcp__claude-flow"
    ...
    if (!dryRun) {
      await Deno.writeTextFile(`${workingDir}/${FILE_PATHS.MCP_CONFIG}`, JSON.stringify(mcpConfig, null, 2));
    ...
    const claudeFlowConfig: ClaudeFlowConfig = {
      "features": {
    ...
    if (!dryRun) {
      await Deno.writeTextFile(`${workingDir}/${FILE_PATHS.CLAUDE_FLOW_CONFIG}`, JSON.stringify(claudeFlowConfig, null, 2));
...
`;
    ...
    if (!dryRun) {
      ...
      await Deno.writeTextFile(`${workingDir}/${FILE_PATHS.UNIX_WRAPPER}`, unixWrapper);
      await fs.chmod(`${workingDir}/${FILE_PATHS.UNIX_WRAPPER}`, INIT_CONSTANTS.UNIX_EXECUTABLE_PERMISSIONS);
      
      // Windows wrapper
      await Deno.writeTextFile(`${workingDir}/${FILE_PATHS.WINDOWS_WRAPPER}`, createWrapperScript('windows'));
      
      // PowerShell wrapper
      await Deno.writeTextFile(`${workingDir}/${FILE_PATHS.POWERSHELL_WRAPPER}`, createWrapperScript('powershell'));
    ...
    for (const helper of helpers) {
      if (!dryRun) {
        ...
        if (content) {
          ...
          await fs.chmod(`${claudeDir}/helpers/${helper}`, INIT_CONSTANTS.UNIX_EXECUTABLE_PERMISSIONS);
        }
      }
    }
    ...
    const gitignoreResult: GitignoreUpdateResult = await updateGitignore(workingDir, force, dryRun);
    if (gitignoreResult.success) {
      if (!dryRun) {
