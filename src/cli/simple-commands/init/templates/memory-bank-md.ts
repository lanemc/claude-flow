// memory-bank-md.ts - Memory Bank templates with TypeScript type safety

// Type definitions for memory bank configuration
export interface MemoryConfig {
  readonly backend: 'json' | 'sqlite' | 'mongodb';
  readonly path: string;
  readonly cacheSize: number;
  readonly indexing: boolean;
  readonly namespaces: readonly string[];
  readonly retentionPolicy: RetentionPolicy;
  readonly batchtools?: BatchtoolsMemoryConfig;
  readonly performance?: PerformanceMemoryConfig;
}
...
export interface RetentionPolicy {
  ...
  readonly batchtools?: string;
}

export interface BatchtoolsMemoryConfig {
  readonly enabled: boolean;
  readonly maxConcurrent: number;
  readonly batchSize: number;
  readonly parallelIndexing: boolean;
  readonly concurrentBackups: boolean;
}

export interface PerformanceMemoryConfig {
  readonly enableParallelAccess: boolean;
  readonly concurrentQueries: number;
  readonly batchWriteSize: number;
  readonly parallelIndexUpdate: boolean;
}
...
export type MemoryBackend = 'json' | 'sqlite' | 'mongodb';
export type MemoryNamespace = 'default' | 'agents' | 'tasks' | 'sessions' | 'sparc' | 'batchtools';

// Create minimal memory bank template
...
// Create full memory bank template with comprehensive configuration
...
export async function createOptimizedMemoryBankMd(): Promise<string> {
...
export function createMemoryConfig(options: Partial<MemoryConfig> = {}): MemoryConfig {
  ...
  return {
    ...defaultConfig,
    ...options,
    namespaces: options.namespaces || defaultConfig.namespaces,
    retentionPolicy: {
      ...defaultConfig.retentionPolicy,
      ...options.retentionPolicy
    }
  };
}

// Batchtools configuration factory
export function createBatchtoolsMemoryConfig(options: Partial<BatchtoolsMemoryConfig> = {}): BatchtoolsMemoryConfig {
  return {
    enabled: options.enabled ?? false,
    maxConcurrent: options.maxConcurrent ?? 10,
    batchSize: options.batchSize ?? 100,
    parallelIndexing: options.parallelIndexing ?? true,
    concurrentBackups: options.concurrentBackups ?? true
  };
}

// Performance configuration factory
export function createPerformanceMemoryConfig(options: Partial<PerformanceMemoryConfig> = {}): PerformanceMemoryConfig {
  return {
    enableParallelAccess: options.enableParallelAccess ?? true,
    concurrentQueries: options.concurrentQueries ?? 20,
    batchWriteSize: options.batchWriteSize ?? 50,
    parallelIndexUpdate: options.parallelIndexUpdate ?? true
  };
}

// Template factory with type safety
export function createMemoryTemplate(
  type: 'minimal' | 'full' | 'optimized',
  config?: Partial<MemoryConfig>
): MemoryTemplate {
  const finalConfig = createMemoryConfig(config);

  switch (type) {
    case 'minimal':
      return {
        content: createMinimalMemoryBankMd(),
        type: 'minimal',
        config: finalConfig
      };
    case 'full':
      return {
        content: createFullMemoryBankMd(),
        type: 'full',
        config: finalConfig
      };
    case 'optimized':
      return {
        content: createOptimizedMemoryBankMd(),
        type: 'optimized',
        config: finalConfig
      };
    default:
      throw new Error(`Invalid memory template type: ${type}`);
  }
}

// Utility functions for validation
export function isValidMemoryBackend(backend: string): backend is MemoryBackend {
  const validBackends: MemoryBackend[] = ['json', 'sqlite', 'mongodb'];
  return validBackends.includes(backend as MemoryBackend);
}

export function isValidMemoryNamespace(namespace: string): namespace is MemoryNamespace {
  const validNamespaces: MemoryNamespace[] = ['default', 'agents', 'tasks', 'sessions', 'sparc', 'batchtools'];
  return validNamespaces.includes(namespace as MemoryNamespace);
}

// Retention policy parsing utility
export function parseRetentionPeriod(period: string): number {
  ...
  switch (unit) {
    ...
    default:
      throw new Error(`Invalid retention period unit: ${unit}`);
  }
}
...
export interface MemoryStats {
  ...
  readonly performance?: {
    ...
    readonly batchOperationCount: number;
  };
}