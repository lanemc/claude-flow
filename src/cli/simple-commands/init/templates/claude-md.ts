...
// Create full CLAUDE.md template with comprehensive configuration
...
// Create SPARC CLAUDE.md template
export function createSparcClaudeMd(): string {
...
export async function createOptimizedSparcClaudeMd(): Promise<string> {
...
export function createClaudeMdTemplate(config: Partial<ClaudeMdConfig> = {}): ClaudeMdTemplate {
  const defaultConfig: ClaudeMdConfig = {
  ...
  if (finalConfig.sparcSupport && finalConfig.batchtools) {
    return {
      content: createOptimizedSparcClaudeMd(),
      type: 'optimized-sparc'
    } as const;
  } else if (finalConfig.sparcSupport) {
    return {
      content: createSparcClaudeMd(),
      type: 'sparc'
    } as const;
  } else if (finalConfig.batchOperations) {
    return {
      content: createFullClaudeMd(),
      type: 'full'
    } as const;
  } else {
    return {
      content: createMinimalClaudeMd(),
      type: 'minimal'
    } as const;
  }
}