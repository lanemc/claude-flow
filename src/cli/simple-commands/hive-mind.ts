...
import ora from 'ora';
import { args, cwd, exit, writeTextFile, readTextFile } from '../node-compat.js';
// @ts-expect-error - mkdirAsync is exported but not typed properly
...
// Type definitions for Hive Mind
...
export interface MCPWrapper {
  searchMemory(namespace: string, query: string): Promise<unknown>;
  storeMemory(namespace: string, key: string, value: unknown, category?: string): Promise<unknown>;
}
 ...
 */
async function spawnSwarm(args: string[], flags: HiveMindFlags): Promise<void> {
 ...
 */
async function showStatus(flags: HiveMindFlags): Promise<void> {
  try {
    ...
    for (const swarm of swarms as SwarmRow[]) {
      ...
      const queen = agentList.find(a => a.role === 'queen');
      const workers = agentList.filter(a => a.role === 'worker');
      ...
      console.log('\n' + chalk.bold('Tasks:'));
      const stats = taskStats as TaskStats;
      console.log(`  Total: ${stats.total}`);
      console.log(`  Completed: ${chalk.green(stats.completed)}`);
      console.log(`  In Progress: ${chalk.yellow(stats.in_progress)}`);
      ...
      console.log(`  Total: ${(consensusCount as TaskCount).count}`);
    }
 ...
 */
async function showConsensus(flags: HiveMindFlags): Promise<void> {
  try {
    const dbPath = path.join(cwd(), '.hive-mind', 'hive.db');
    const db = new Database(dbPath);
    ...
    (decisions as ConsensusRow[]).forEach(decision => {
      ...
      if (decision.votes) {
        ...
        if (votes.for !== undefined || votes.against !== undefined || votes.abstain !== undefined) {
          ...
          if (votes.details && Array.isArray(votes.details)) {
            console.log('  - details:');
            votes.details.forEach((detail, _index) => {
 ...
 */
async function showMetrics(flags: HiveMindFlags): Promise<void> {
  try {
    const dbPath = path.join(cwd(), '.hive-mind', 'hive.db');
    const db = new Database(dbPath);
    ...
    if (taskBreakdown.length > 0) {
      ...
      taskBreakdown.forEach(status => {
        const percentage = stats.total_tasks > 0 
          ? (((status as TaskCount).count / stats.total_tasks) * 100).toFixed(1)
          : '0';
        const statusObj = status as any;
        const statusColor = 
          statusObj.status === 'completed' ? 'green' :
          statusObj.status === 'in_progress' ? 'yellow' :
          statusObj.status === 'failed' ? 'red' : 'gray';
        console.log(`  ${chalk[statusColor](statusObj.status.charAt(0).toUpperCase() + statusObj.status.slice(1))}: ${statusObj.count} (${percentage}%)`);
      });
    }
    ...
    try {
      ...
      if (hasCompletedAt && (hasCompletedAt as TaskCount).count > 0) {
    ...
    if (agentPerf.length > 0) {
      ...
      agentPerf.forEach((agent, _index) => {
    ...
    if (swarmPerf.length > 0) {
      ...
      (swarmPerf as SwarmSummary[]).forEach(swarm => {
    ...
    let avgTaskTime: { avg_minutes: any } = { avg_minutes: null };
    try {
      // Check if completed_at exists
      const hasCompletedAt = db.prepare(`
        SELECT COUNT(*) as count FROM pragma_table_info('tasks') 
    ...
    if (avgTaskTime.avg_minutes) {
      ...
      if (agentTypePerf.length > 0) {
        ...
        agentTypePerf.forEach(type => {
          ...
          const successRate = typeObj.total_tasks > 0 
            ? ((typeObj.completed_tasks / typeObj.total_tasks) * 100).toFixed(1)
            : '0';
          console.log(`  ${typeObj.type.charAt(0).toUpperCase() + typeObj.type.slice(1)}: ${typeObj.completed_tasks}/${typeObj.total_tasks} (${successRate}%)`);
          if (typeObj.avg_completion_minutes) {
            console.log(`    Average time: ${typeObj.avg_completion_minutes.toFixed(1)} minutes`);
          }
        });
      }
    }
 ...
 */
async function manageMemoryWizard(): Promise<void> {
 ...
 */
export async function hiveMindCommand(args: string[], flags: HiveMindFlags): Promise<void> {
 ...
 */
async function storeMemoryWizard(): Promise<void> {
  try {
    const answers = await inquirer.prompt([
      {
        type: 'input',
 ...
 */
async function exportMemoryBackup(): Promise<void> {
  try {
 ...
 */
async function spawnClaudeCodeInstances(swarmId: string, swarmName: string, objective: string, workers: Agent[], flags: HiveMindFlags): Promise<void> {
 ...
 */
function generateHiveMindPrompt(swarmId: string, swarmName: string, objective: string, workers: Agent[], workerGroups: WorkerGroup, flags: HiveMindFlags): string {
  const currentTime: string = new Date().toISOString();
  const workerTypes: string[] = Object.keys(workerGroups);
  const queenType: string = flags.queenType || 'strategic';
  const consensusAlgorithm: string = flags.consensus || 'majority';
 ...
 */
function generateCoordinationInstructions(swarmId: string, swarmName: string, objective: string, workers: Agent[]): unknown {
  return {
    swarmId,
    swarmName,
    objective,
 ...
 */
function groupWorkersByType(workers: Agent[]): WorkerGroup {
 ...
 */
function createClaudeCodeSpawnCommand(swarmId: string, swarmName: string, objective: string, workerType: string, typeWorkers: Agent[], instructions: unknown): ClaudeCodeSpawnCommand {
 ...
 */
async function getMcpWrapper(): Promise<MCPWrapper> {
