  ...
  .action(async (options: StartOptions) => {
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Initialization timeout')), (Number(options.timeout) || 30) * 1000)
      );
      ...
      if (options.ui) {
        ...
        try {
          const { ClaudeCodeWebServer } = await import('../../simple-commands/web-server');
          
          // Start the web server
          console.log(chalk.blue('Starting Web UI server...'));
          const webServer = new ClaudeCodeWebServer(Number(options.port));
