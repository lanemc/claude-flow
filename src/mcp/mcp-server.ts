...
export class ClaudeFlowMCPServer implements ClaudeFlowMCPServerInterface {
  ...
  async handleMessage(message: MCPMessage): Promise<MCPResponse> {
    try {
      ...
      return this.createErrorResponse(message.id!, -32603, 'Internal error', err.message);
    }
  }
  ...
  async handleToolCall(id: string | number, params: { name: string; arguments: Record<string, any> }): Promise<MCPResponse> {
    ...
    try {
      ...
      const err = error as Error;
      return this.createErrorResponse(id, -32000, 'Tool execution failed', err.message);
    }
  }
  ...
  async handleResourceRead(id: string | number, params: { uri: string }): Promise<MCPResponse> {
    const { uri } = params;
    ...
    try {
      ...
      const err = error as Error;
      return this.createErrorResponse(id, -32000, 'Resource read failed', err.message);
    }
  }

  async executeTool(name: string, args: Record<string, any>): Promise<MCPToolExecutionResult> {
  ...
  async readResource(uri: string): Promise<SwarmResourceContent | AgentResourceContent | ModelResourceContent | PerformanceResourceContent> {
    switch (uri) {
  ...
  async handleMemoryUsage(args: MemoryOperation): Promise<MemoryStoreResult | MemoryRetrieveResult | MemoryListResult | MemorySearchResult | { success: false; error: string; timestamp: string }> {
    if (!this.memoryStore) {
      return {
        success: false,
    ...
    try {
      ...
      const err = error as Error;
      console.error(`[${new Date().toISOString()}] ERROR [claude-flow-mcp] Memory operation failed:`, err);
      return {
        success: false,
        error: err.message,
        timestamp: new Date().toISOString()
  ...
  async handleMemorySearch(args: { pattern: string; namespace?: string; limit?: number }): Promise<MemorySearchResult | { success: false; error: string; timestamp: string }> {
    if (!this.memoryStore) {
      return {
        success: false,
        error: 'Memory system not initialized',
    ...
    try {
      ...
      const err = error as Error;
      console.error(`[${new Date().toISOString()}] ERROR [claude-flow-mcp] Memory search failed:`, err);
      return {
        success: false,
        error: err.message,
        timestamp: new Date().toISOString()
    ...
    for (const line of lines) {
      if (line.trim()) {
        try {
          ...
          const err = error as Error;
          console.error(`[${new Date().toISOString()}] ERROR [claude-flow-mcp] Failed to parse message:`, err.message);
        }
      }
    }
  });
